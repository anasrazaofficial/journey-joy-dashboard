import img_avatarTable from "./avatar-table.png";
import img_avatar from "./avatar.png";
import img_check from "./check.svg";
import img_chevron from "./chevron.svg";
import img_dashboardChart from "./dashboard-chart.png";
import img_dashboardStatCardRevenue from "./dashboard-stat-card-revenue.svg";
import img_dashboardStatCardSubscription from "./dashboard-stat-card-subscription.svg";
import img_logo from "./logo.svg";
import img_logout from "./logout.svg";
import img_search from "./search.svg";
import img_totalCompanyProfit from "./total-company-profit.svg";
import img_totalRevenue from "./total-revenue.svg";


export const
    avatarTable = img_avatarTable,
    avatar = img_avatar,
    check = img_check,
    chevron = img_chevron,
    dashboardChart = img_dashboardChart,
    dashboardStatCardRevenue = img_dashboardStatCardRevenue,
    dashboardStatCardSubscription = img_dashboardStatCardSubscription,
    logo = img_logo,
    logout = img_logout,
    search = img_search,
    totalCompanyProfit = img_totalCompanyProfit,
    totalRevenue = img_totalRevenue;